  ...
  return `<div style="background:linear-gradient(135deg,#F0F4FA 0%,#E8F0F9 100%);border:1.5px solid #2E5FA3;border-radius:10px;padding:12px 14px;margin-bottom:14px">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:${sel.length>0?'8px':'0'};flex-wrap:wrap">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button class="btn btn-sm" style="background:white;border:1px solid #2E5FA3;color:#2E5FA3;font-weight:700" onclick="openEmployeeFilterModal()">
          👥 ${allSelected ? 'Employees' : 'Edit'}
        </button>
        ${(!allSelected||projF||locF||branchF||empDeptF||taskDeptF||workTypeF||statusF||categoryF||subcatF) ? `<button class="btn btn-sm" style="background:white;border:1px solid #C53030;color:#C53030;font-weight:700" onclick="clearAllReportFilters()">✕ Clear All</button>` : ''}
      </div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
      ...
      ${allBranches.length>0?`<select onchange="window.setGlobalBranchFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #1565C0;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
      ...
      ${allEmpDepts.length>0?`<select onchange="window.setGlobalEmpDeptFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #6A1B9A;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
      ...
      ${allTaskDepts.length>0?`<select onchange="window.setGlobalTaskDeptFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #00897B;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
      ...
      <select onchange="window.setGlobalProjectFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #2E5FA3;border-radius:6px;font-size:12px;background:white;color:#1B3A6B">
        <option value="">📁 All Projects</option>
        ${allProjects.map(p=>`<option value="${escapeHtml(p)}" ${p===projF?"selected":""}>${escapeHtml(p)}</option>`).join("")}
      </select>
      <select onchange="window.setGlobalLocationFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #2E5FA3;border-radius:6px;font-size:12px;background:white;color:#1B3A6B">
        <option value="">📍 All Locations</option>
        ${allLocations.map(l=>`<option value="${escapeHtml(l)}" ${l===locF?"selected":""}>${escapeHtml(l)}</option>`).join("")}
      </select>
      ${allWorkTypes.length>0?`<select onchange="window.setGlobalWorkTypeFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #E65100;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
        <option value="">🔧 All Work Types</option>
        ${allWorkTypes.map(w=>`<option value="${escapeHtml(w)}" ${w===workTypeF?"selected":""}>${escapeHtml(w)}</option>`).join("")}
      </select>`:''}
      ${allStatuses.length>0?`<select onchange="window.setGlobalTaskStatusFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #C2185B;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
        <option value="">📊 All Statuses</option>
        ${allStatuses.map(s2=>`<option value="${escapeHtml(s2)}" ${s2===statusF?"selected":""}>${escapeHtml(s2)}</option>`).join("")}
      </select>`:''}
      ${allCategories.length>0?`<select onchange="window.setGlobalCategoryFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #00695C;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
        <option value="">📁 All Categories</option>
        ${allCategories.map(c=>`<option value="${escapeHtml(c)}" ${c===categoryF?"selected":""}>${escapeHtml(c)}</option>`).join("")}
      </select>`:''}
      ${allSubcats.length>0?`<select onchange="window.setGlobalSubcategoryFilter(this.value)" style="flex:1;min-width:140px;padding:7px 10px;border:1px solid #4527A0;border-radius:6px;font-size:12px;background:white;color:#1B3A6B;font-weight:600">
...
async function subscribeData(){
  ...
  renderRoot(`<div class="loader"><div class="loader-spin"></div></div>`);
...
function renderTab(){
  ...
  const fn={
    ...
    "Travel":renderTravel,"Leaves":renderLeaves,"HR Report":renderHRReport,"Technical Report":renderTechReport,"Reports":renderFlexReports,
