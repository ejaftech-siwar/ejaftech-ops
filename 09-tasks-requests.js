          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end">
              ...
              ${isAdmin()||isHR()?`<select onchange="updateRequestStatus('${r.id}',this.value)" style="padding:5px 10px;border:1px solid var(--line);border-radius:8px;font-size:12px;font-weight:600">
                ${(()=>{const opts=getReqStatusList();const has=opts.some(o=>o.value===r.status);const all=has?opts:[{value:r.status,label:prettyStatus(r.status)},...opts];return all.map(o=>`<option value="${escapeHtml(o.value)}" ${r.status===o.value?"selected":""}>${escapeHtml(o.label)}</option>`).join("");})()}
              </select>`:""}
              ${isAdmin()?`<button class="btn btn-sm btn-danger" onclick="delRequest('${r.id}')">${ICN.del}</button>`:""}
              ${canUseWhatsApp() && (waGetSettings().triggers||[]).includes("clientRequests")?`<button class="btn btn-sm" style="background:#25D366;color:white;border:none;font-weight:700" onclick='openWaShare(${escapeHtml(JSON.stringify({employee:r.clientName,project:r.project,date:(r.createdAt||"").slice(0,10),resolutionText:r.title+" — "+r.description,description:r.title+" — "+r.description}))})'>📲</button>`:""}${canUseEmail() && (emailGetSettings().triggers||[]).includes("clientRequests")?`<button class="btn btn-sm" style="background:#03308B;color:white;border:none;font-weight:700" onclick='openEmailShare(${escapeHtml(JSON.stringify({employee:r.clientName,project:r.project,date:(r.createdAt||"").slice(0,10),resolutionText:r.title+" — "+r.description,description:r.title+" — "+r.description}))})'>📧</button>`:""}
            </div>
          </div>
        </div>`;
      }).join("")}
